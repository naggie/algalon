...
var Main = function() {
	...
	this.instantiateEntity = function(initial) {
		...
		var path = './entities/'+initial.class

		// require will throw an error if this class does not exist. If
		// the cause is not obvious, I will test first
		if (!entityClasses[initial.class])
			entityClasses[initial.class] = require(path)

		var Class = entityClasses[initial.class]
	...
	this.calculateHealth = function() {
		...
		for (var i in self.states) {
			if (typeof self.states[i].healthy == 'undefined') return
