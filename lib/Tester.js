...
var State = function(initial) {
	...
	this.fail = function(err) {
		...
		if (consecutive_failures == 1)
			self.lastfailedge = new Date()
				...
				&& consecutive_passes >= self.pass_threshold) {
			// failed after passing for a while
			pushEvent()
	...
	this.pass = function() {
		...
		else if (consecutive_passes == self.pass_threshold
				&& consecutive_failures >= self.fail_threshold) {
			// passed after failing for a while
			pushEvent()
