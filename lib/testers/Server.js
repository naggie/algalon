...
var Main = function(config) {
			...
			if (!err && res.statusCode == 200) {
				...
				if(!data.error) {
					...
					if(data['temperature_c'] > 80)
						...
						self.state.fail('Disk almost full')
					else if (data['mem_used']/data['mem_total'] > 0.95)
						self.state.fail('Application memory usage over 95%')
					else if (data['battery_percent'] < 30)
						self.state.fail('Backup power critical')
					else if (data['line_voltage'] < 10)
						self.state.fail('Grid power failure')
					else if (data['line_voltage'] > 253)
						self.state.fail('Mains over-voltage alert: '+data['line_voltage']+'V')
					else if (data['line_voltage'] < 216)
						self.state.fail('Mains under-voltage alert: '+data['line_voltage']+'V')
	...
	this.update = function(data) {
		// too many changes, client only displays days anyway
		data.uptime_days = (data.uptime_secs/84600).toFixed(0)
		delete data.uptime_secs
